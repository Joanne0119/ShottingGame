...
// 控制五角形運動所需的相關變數
bool g_bMoving = false;  // 用於滑鼠變更模型的位置
bool g_bRotating = false;// 用於模型的旋轉
GLfloat g_viewScale = 2.0f; // 平行投影的縮放值

//----------------------------------------------------------------------------
void loadScene(void)
{
	g_penta = new CPenta;
    g_shaderProg = g_penta->setShader("vshader21.glsl","fshader21.glsl");
	g_penta->setScale(glm::vec3(0.45f, 0.45f, 1.0f));
...
void update(float dt)
{
    if (g_bRotating)
    {
        g_angle += 180.0f * dt; // 每秒轉180度
        if (g_angle > 360.0f) g_angle -= 360.0f;
        g_penta->setRotZ(g_angle);
    }
}
