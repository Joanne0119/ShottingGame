...
CTriangle* g_tri = nullptr;
CPenta* g_penta = nullptr;
...
void loadScene(void)
{
	...
	g_quad->setPos(glm::vec3(2.0f, 2.0f, 0.0f));

    g_tri = new CTriangle;
    g_tri->setShaderID(g_shaderProg);
	g_tri->setPos(glm::vec3(-2.0f, 2.0f, 0.0f));

	g_penta = new CPenta[2];
	g_penta[0].setShaderID(g_shaderProg);
	g_penta[0].setPos(glm::vec3(-2.0f, -2.0f, 0.0f));
	g_penta[1].setShaderID(g_shaderProg);
	g_penta[1].setPos(glm::vec3(2.0f, -2.0f, 0.0f));
...
void releaseAll()
{
	if(g_tri != nullptr) delete g_tri;
	if(g_quad != nullptr) delete g_quad;
	if(g_penta != nullptr) delete[] g_penta;
}
